	...
	return (
		<div className="flex self-stretch shrink-0">
			{Array.from({ length: level }).map((_, i) => (
				...
				<div key={i} className="w-3 self-stretch border-r border-border/40" />
			))}
		</div>
	);
}
		...
		>
			...
			>
				...
				<span className="flex-1 min-w-0 flex items-center gap-1">
					...
					{showStatsDisplay && (
						<span className="flex items-center gap-0.5 text-xs font-mono tabular-nums shrink-0 whitespace-nowrap opacity-60">
							{file.additions > 0 && (
